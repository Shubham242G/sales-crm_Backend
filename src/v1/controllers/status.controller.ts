...
export const addStatus = async (
    ...
    try {
      ...
      res.status(201).json({ message: "Status Created", data: status });
...
export const getStatusById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        let existsCheck = await Status.aggregate(pipeline);
        if (!existsCheck || existsCheck.length == 0) {
            throw new Error("Status does not exists");
        }
        existsCheck = existsCheck[0];
        res.status(201).json({
            message: "found specific Status",
            data: existsCheck,
        });
    } catch (error) {
        next(error);
...
export const getAllStatus = async (req: any, res: any, next: any) => {
    try {
        ...
        let StatusArr = await paginateAggregate(Status, pipeline, req.query);

        res.status(201).json({ message: "found all Status", data: StatusArr.data, total: StatusArr.total });
    } catch (error) {
        next(error);
    }
};
...
export const updateStatusById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        let existsCheck = await Status.findById(req.params.id).lean().exec();
        if (!existsCheck) {
            throw new Error("Status does not exists");
        }
        ...
        //     }
        // }
        let Obj = await Status.findByIdAndUpdate(req.params.id, req.body).exec();
...
export const deleteStatusById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        let existsCheck = await Lead.findById(req.params.id).exec();
        if (!existsCheck) {
            throw new Error("Status does not exists or already deleted");
        }
