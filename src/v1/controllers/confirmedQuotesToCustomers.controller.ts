...
import { ConfirmedQuotesToCustomer } from "../../models/confirmedQuotesToCustomers";
...
export const addConfirmedQuotesToCustomer = async (req: Request, res: Response, next: NextFunction) => {
    try {
    ...
    //     }
    // }
        const confirmedQuotesToCustomer = await new ConfirmedQuotesToCustomer({...req.body, status: "Quote sent to customer"}).save();
        res.status(201).json({ message: "Confirmed Quote To Customer Created" });
...
export const getAllConfirmedQuotesToCustomer = async (req: any, res: any, next: any) => {
    try {
        ...
        let ConfirmedQuotesToCustomerArr = await paginateAggregate(ConfirmedQuotesToCustomer, pipeline, req.query);

        res.status(201).json({ message: "found all Device", data: ConfirmedQuotesToCustomerArr.data, total: ConfirmedQuotesToCustomerArr.total });
    } catch (error) {
        next(error);
    }
};

export const getConfirmedQuotesToCustomerById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        let pipeline: PipelineStage[] = [];
        let matchObj: Record<string, any> = {};
        if (req.params.id) {
        ...
        let existsCheck = await ConfirmedQuotesToCustomer.aggregate(pipeline);
...
export const updateConfirmedQuotesToCustomerById = async (req: Request, res: Response, next: NextFunction) => {
    try {


  

        let existsCheck = await ConfirmedQuotesToCustomer.findById(req.params.id).lean().exec();
        if (!existsCheck) {
            throw new Error("Quote From Vendor does not exists");
        }
        ...
        //     }
        let Obj = await ConfirmedQuotesToCustomer.findByIdAndUpdate(req.params.id, { ...req.body, status: "Under negotiation with client" }).exec();
        await Obj?.save();
...
export const deleteConfirmedQuotesToCustomerById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        let existsCheck = await ConfirmedQuotesToCustomer.findById(req.params.id).exec();
        if (!existsCheck) {
            throw new Error("Quote from Vendor does not exists or already deleted");
        }
        await ConfirmedQuotesToCustomer.findByIdAndDelete(req.params.id).exec();
...
export const downloadExcelQuotes = async (
    ...
    return ExportService.downloadFile(req, res, next, {
      model: ConfirmedQuotesToCustomer,
...
//       const customerQuote = await ConfirmedQuotesToCustomer.findById(quoteId);
//       if (!customerQuote) {
